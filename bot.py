...
# OpenAI-Based Categorization + Formatting
async def format_tweet_message(tweet, tweet_link):
    try:
        response = await openai_client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "You are an assistant that categorizes and reformats tweets for Telegram. Identify the category (partnership, announcement, or AMA) and rewrite the tweet in a structured, engaging format."},
                {"role": "user", "content": f"Categorize and rewrite this tweet for Telegram:\n\n{tweet}\n\nProvide a structured message including category, emojis, and a call to action. Keep it concise and engaging. End with this tweet link: {tweet_link}"}
            ]
        )
    ...
    while True:
        ...
        if tweet:
            ...
            if current_tweet_hash in ignored_hashes or current_tweet_hash == last_tweet_hash:
                ...
                formatted_message = await format_tweet_message(tweet, tweet_link)
